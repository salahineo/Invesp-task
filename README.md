# Invesp Test Task

### Task

This is a JS functionality to show product images dynamically in a popup carousel, built as a part of a test task for Invesp.

### Author

This task is developed by Mohamed Salah (salahineo.personal@gmail.com). You can found the author's contact information at the end of this document.

### Notes

- This application developed on this page (https://www.pamono.com/furniture)
- This application developed using the User JavaScript and CSS extension for Google Chrome
- You should load and activate the needed modules in the extension to make this work as expected (kindly have a look at the next screenshot):
  - LightGallery CSS: https://cdnjs.cloudflare.com/ajax/libs/lightgallery/2.7.2/css/lightgallery-bundle.min.css
  - LightGallery JS: https://cdnjs.cloudflare.com/ajax/libs/lightgallery/2.7.2/lightgallery.min.js
  - LightGallery Thumbnail Plugin: https://cdnjs.cloudflare.com/ajax/libs/lightgallery/2.7.2/plugins/thumbnail/lg-thumbnail.min.js

<p align="center">
<img src="assets/modules.png" alt="Modules of the plugin">
</p>


### Demonstration Video
https://github.com/user-attachments/assets/daa4879d-45dd-4822-90ad-b4a1c1421857



> Thank you for taking time review this task 

---

### Contact

<a href="https://github.com/salahineo"><img src="https://img.shields.io/badge/-GitHub-181717?style=flat&logo=github&logoColor=ffffff"/></a>
<a href="https://linkedin.com/in/salahineo"><img src="https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=ffffff"/></a>
<a href="https://salahineo.github.io/salahineo/"><img src="https://img.shields.io/badge/-Website-0563BB?style=flat&logo=data:image/png;base64,&logoColor=ffffff"/></a>
<a href="mailto:salahineo.work@gmail.com"><img src="https://img.shields.io/badge/-Email-2b3752?style=flat&logo=data:image/png;base64,&logoColor=ffffff"/></a>
